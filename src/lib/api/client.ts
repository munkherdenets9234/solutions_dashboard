...
// Components, Route Handlers, Server Actions, or other server-side code.
...
function apiKey() {
  ...
  if (!key) {
    throw new Error('TENANT_API_KEY is not set — add it to .env.local (see .env.example).')
  }
  return key
}
...
// Admin data is live operational data (bookings, statuses, inventory) — always
// bypass the Next.js Data Cache, unlike the public site's 300s revalidate.
export function apiGet<T>(path: string, searchParams?: Record<string, string | number | boolean | undefined>, token?: string) {
  return request<T>(`${path}${toQueryString(searchParams)}`, { method: 'GET', cache: 'no-store' }, token)
}
